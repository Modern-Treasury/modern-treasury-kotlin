...
private constructor(
    ...
    fun cat(): Cat? = cat
    fun dog(): Dog? = dog
    fun cats(): List<Cat>? = cats
    fun petName(): String? = petName
    ...
    fun validate(): ExampleOneOf = apply {
        if (!validated) {
            if (cat == null && dog == null && cats == null && petName == null) {
                throw ModernTreasuryInvalidDataException("Unknown ExampleOneOf: $_json")
            }
            cat?.validate()
            dog?.validate()
    ...
    interface Visitor<out T> {
        ...
        fun visitCats(cats: List<Cat>): T

        fun visitPetName(petName: String): T

        fun unknown(json: JsonValue?): T {
            throw ModernTreasuryInvalidDataException("Unknown ExampleOneOf: $json")
        }
    }
