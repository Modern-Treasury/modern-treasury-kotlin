...
private constructor(
    ...
    fun getNextPageParams(): LedgerAccountPayoutListParams? {
        ...
        return LedgerAccountPayoutListParams.builder()
            .from(params)
            .afterCursor(afterCursor())
            .build()
    }

    suspend fun getNextPage(): LedgerAccountPayoutListPageAsync? {
        return getNextPageParams()?.let { ledgerAccountPayoutsService.list(it) }
    }
    ...
    constructor(
        ...
        @JsonProperty("items") fun _items(): JsonField<List<LedgerAccountPayout>>? = items
    ...
    ) : Flow<LedgerAccountPayout> {

        override suspend fun collect(collector: FlowCollector<LedgerAccountPayout>) {
            var page = firstPage
            var index = 0
            while (true) {
