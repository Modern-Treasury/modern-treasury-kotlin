...
class PaymentActionCreateResponse @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
     ...
     * ```kotlin
     * val myObject: MyClass = paymentActionCreateResponse.details().convert(MyClass::class.java)
     * ```
     */
    ...
    class Builder internal constructor() {
        ...
        internal fun from(paymentActionCreateResponse: PaymentActionCreateResponse) =
            apply {
     ...
     *   expected type.
     */
    fun validate(): PaymentActionCreateResponse =
        apply {
            if (validated) {
              return@apply
            }
    ...
    override fun toString() = "PaymentActionCreateResponse{id=$id, actionableId=$actionableId, actionableType=$actionableType, createdAt=$createdAt, details=$details, internalAccountId=$internalAccountId, liveMode=$liveMode, object_=$object_, status=$status, type=$type, updatedAt=$updatedAt, additionalProperties=$additionalProperties}"
}
