...
class ExternalAccountCreateParams private constructor(
    ...
    class Builder internal constructor() {
        ...
        private var body: ExternalAccountCreateRequest.Builder = ExternalAccountCreateRequest.builder()
         ...
         * - etc.
         */
        fun body(body: ExternalAccountCreateRequest) =
            apply {
    ...
    class ExternalAccountCreateRequest @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
        ...
        companion object {
            ...
            /**
             * Returns a mutable builder for constructing an instance of [ExternalAccountCreateRequest].
             *
             * The following fields are required:
             *
             * ```kotlin
        ...
        class Builder internal constructor() {
            ...
            internal fun from(externalAccountCreateRequest: ExternalAccountCreateRequest) =
                apply {
                    counterpartyId = externalAccountCreateRequest.counterpartyId
                    accountDetails = externalAccountCreateRequest.accountDetails.map { it.toMutableList() }
                    accountType = externalAccountCreateRequest.accountType
                    contactDetails = externalAccountCreateRequest.contactDetails.map { it.toMutableList() }
                    ...
                    routingDetails = externalAccountCreateRequest.routingDetails.map { it.toMutableList() }
                    additionalProperties = externalAccountCreateRequest.additionalProperties.toMutableMap()
                }
         ...
         *   expected type.
         */
        fun validate(): ExternalAccountCreateRequest =
            apply {
                if (validated) {
                  return@apply
                }
         ...
         */
        internal fun validity(): Int = (if (counterpartyId.asKnown() == null) 0 else 1) + (accountDetails.asKnown()?.sumOf { it.validity().toInt() } ?: 0) + (accountType.asKnown()?.validity() ?: 0) + (contactDetails.asKnown()?.sumOf { it.validity().toInt() } ?: 0) + (if (externalId.asKnown() == null) 0 else 1) + (ledgerAccount.asKnown()?.validity() ?: 0) + (metadata.asKnown()?.validity() ?: 0) + (if (name.asKnown() == null) 0 else 1) + (partyAddress.asKnown()?.validity() ?: 0) + (if (partyIdentifier.asKnown() == null) 0 else 1) + (if (partyName.asKnown() == null) 0 else 1) + (partyType.asKnown()?.validity() ?: 0) + (if (plaidProcessorToken.asKnown() == null) 0 else 1) + (routingDetails.asKnown()?.sumOf { it.validity().toInt() } ?: 0)
        ...
        override fun equals(other: Any?): Boolean {
          ...
          return other is ExternalAccountCreateRequest && counterpartyId == other.counterpartyId && accountDetails == other.accountDetails && accountType == other.accountType && contactDetails == other.contactDetails && externalId == other.externalId && ledgerAccount == other.ledgerAccount && metadata == other.metadata && name == other.name && partyAddress == other.partyAddress && partyIdentifier == other.partyIdentifier && partyName == other.partyName && partyType == other.partyType && plaidProcessorToken == other.plaidProcessorToken && routingDetails == other.routingDetails && additionalProperties == other.additionalProperties
        }

        private val hashCode: Int by lazy { Objects.hash(counterpartyId, accountDetails, accountType, contactDetails, externalId, ledgerAccount, metadata, name, partyAddress, partyIdentifier, partyName, partyType, plaidProcessorToken, routingDetails, additionalProperties) }

        override fun hashCode(): Int = hashCode

        override fun toString() = "ExternalAccountCreateRequest{counterpartyId=$counterpartyId, accountDetails=$accountDetails, accountType=$accountType, contactDetails=$contactDetails, externalId=$externalId, ledgerAccount=$ledgerAccount, metadata=$metadata, name=$name, partyAddress=$partyAddress, partyIdentifier=$partyIdentifier, partyName=$partyName, partyType=$partyType, plaidProcessorToken=$plaidProcessorToken, routingDetails=$routingDetails, additionalProperties=$additionalProperties}"
    }
    ...
    override fun equals(other: Any?): Boolean {
      ...
      return other is ExternalAccountCreateParams && body == other.body && additionalHeaders == other.additionalHeaders && additionalQueryParams == other.additionalQueryParams
    }
