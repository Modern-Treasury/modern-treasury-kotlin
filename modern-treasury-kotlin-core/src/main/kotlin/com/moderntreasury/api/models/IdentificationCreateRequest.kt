...
class IdentificationCreateRequest @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
     ...
     */
    internal fun validity(): Int = (if (idNumber.asKnown() == null) 0 else 1) + (idType.asKnown()?.validity() ?: 0) + (documents.asKnown()?.sumOf { it.validity().toInt() } ?: 0) + (if (expirationDate.asKnown() == null) 0 else 1) + (if (issuingCountry.asKnown() == null) 0 else 1) + (if (issuingRegion.asKnown() == null) 0 else 1)
