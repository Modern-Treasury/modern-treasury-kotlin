...
class LedgerEntryCreateRequest @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
    ...
    class Builder internal constructor() {
        ...
        internal fun from(ledgerEntryCreateRequest: LedgerEntryCreateRequest) =
            apply {
                ...
                showResultingLedgerAccountBalances = ledgerEntryCreateRequest.showResultingLedgerAccountBalances
     ...
     *   expected type.
     */
    fun validate(): LedgerEntryCreateRequest =
        apply {
            if (validated) {
              return@apply
            }
     ...
     */
    internal fun validity(): Int = (direction.asKnown()?.validity() ?: 0) + (if (ledgerAccountId.asKnown() == null) 0 else 1) + (if (amount.asKnown() == null) 0 else 1) + (if (amountString.asKnown() == null) 0 else 1) + (availableBalanceAmount.asKnown()?.validity() ?: 0) + (if (effectiveAt.asKnown() == null) 0 else 1) + (if (lockVersion.asKnown() == null) 0 else 1) + (metadata.asKnown()?.validity() ?: 0) + (pendingBalanceAmount.asKnown()?.validity() ?: 0) + (postedBalanceAmount.asKnown()?.validity() ?: 0) + (if (showResultingLedgerAccountBalances.asKnown() == null) 0 else 1)
    ...
    override fun equals(other: Any?): Boolean {
      ...
      return other is LedgerEntryCreateRequest && direction == other.direction && ledgerAccountId == other.ledgerAccountId && amount == other.amount && amountString == other.amountString && availableBalanceAmount == other.availableBalanceAmount && effectiveAt == other.effectiveAt && lockVersion == other.lockVersion && metadata == other.metadata && pendingBalanceAmount == other.pendingBalanceAmount && postedBalanceAmount == other.postedBalanceAmount && showResultingLedgerAccountBalances == other.showResultingLedgerAccountBalances && additionalProperties == other.additionalProperties
    }

    private val hashCode: Int by lazy { Objects.hash(direction, ledgerAccountId, amount, amountString, availableBalanceAmount, effectiveAt, lockVersion, metadata, pendingBalanceAmount, postedBalanceAmount, showResultingLedgerAccountBalances, additionalProperties) }

    override fun hashCode(): Int = hashCode

    override fun toString() = "LedgerEntryCreateRequest{direction=$direction, ledgerAccountId=$ledgerAccountId, amount=$amount, amountString=$amountString, availableBalanceAmount=$availableBalanceAmount, effectiveAt=$effectiveAt, lockVersion=$lockVersion, metadata=$metadata, pendingBalanceAmount=$pendingBalanceAmount, postedBalanceAmount=$postedBalanceAmount, showResultingLedgerAccountBalances=$showResultingLedgerAccountBalances, additionalProperties=$additionalProperties}"
}
