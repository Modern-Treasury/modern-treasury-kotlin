...
class ForeignExchangeQuote @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
     ...
     */
    internal fun validity(): Int = (if (id.asKnown() == null) 0 else 1) + (if (createdAt.asKnown() == null) 0 else 1) + (if (effectiveAt.asKnown() == null) 0 else 1) + (if (expiresAt.asKnown() == null) 0 else 1) + (if (foreignExchangeIndicator.asKnown() == null) 0 else 1) + (foreignExchangeRate.asKnown()?.validity() ?: 0) + (if (internalAccountId.asKnown() == null) 0 else 1) + (if (liveMode.asKnown() == null) 0 else 1) + (metadata.asKnown()?.validity() ?: 0) + (if (object_.asKnown() == null) 0 else 1) + (if (updatedAt.asKnown() == null) 0 else 1) + (if (vendorId.asKnown() == null) 0 else 1)
