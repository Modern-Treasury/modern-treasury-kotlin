...
class Ledger @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
    ...
    class Builder internal constructor() {
        ...
        internal fun from(ledger: Ledger) =
            apply {
                id = ledger.id
                createdAt = ledger.createdAt
                description = ledger.description
                discardedAt = ledger.discardedAt
                liveMode = ledger.liveMode
                metadata = ledger.metadata
                name = ledger.name
                object_ = ledger.object_
     ...
     */
    internal fun validity(): Int = (if (id.asKnown() == null) 0 else 1) + (if (createdAt.asKnown() == null) 0 else 1) + (if (description.asKnown() == null) 0 else 1) + (if (discardedAt.asKnown() == null) 0 else 1) + (if (liveMode.asKnown() == null) 0 else 1) + (metadata.asKnown()?.validity() ?: 0) + (if (name.asKnown() == null) 0 else 1) + (if (object_.asKnown() == null) 0 else 1) + (if (updatedAt.asKnown() == null) 0 else 1)
    ...
    override fun equals(other: Any?): Boolean {
      ...
      return other is Ledger && id == other.id && createdAt == other.createdAt && description == other.description && discardedAt == other.discardedAt && liveMode == other.liveMode && metadata == other.metadata && name == other.name && object_ == other.object_ && updatedAt == other.updatedAt && additionalProperties == other.additionalProperties
    }

    private val hashCode: Int by lazy { Objects.hash(id, createdAt, description, discardedAt, liveMode, metadata, name, object_, updatedAt, additionalProperties) }

    override fun hashCode(): Int = hashCode

    override fun toString() = "Ledger{id=$id, createdAt=$createdAt, description=$description, discardedAt=$discardedAt, liveMode=$liveMode, metadata=$metadata, name=$name, object_=$object_, updatedAt=$updatedAt, additionalProperties=$additionalProperties}"
}
