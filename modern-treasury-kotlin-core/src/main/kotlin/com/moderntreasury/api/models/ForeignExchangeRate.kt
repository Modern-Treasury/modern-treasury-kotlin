...
class ForeignExchangeRate @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
    ...
    private constructor(
        ...
        @JsonProperty("rate_string") @ExcludeMissing rateString: JsonField<String> = JsonMissing.of(),
    ...
    @ExcludeMissing
    fun _exponent(): JsonField<Long> = exponent
    ...
    class Builder internal constructor() {
        ...
        internal fun from(foreignExchangeRate: ForeignExchangeRate) =
            apply {
         ...
         *
         * You should usually call [Builder.exponent] with a well-typed [Long] value instead. This method is primarily for setting the field to an undocumented or not yet
         * supported value.
         */
        fun exponent(exponent: JsonField<Long>) =
            apply {
         ...
         *
         * You should usually call [Builder.rateString] with a well-typed [String] value instead. This method is primarily for setting the field to an undocumented or not yet
         * supported value.
         */
        fun rateString(rateString: JsonField<String>) =
            apply {
     ...
     *   expected type.
     */
    fun validate(): ForeignExchangeRate =
        apply {
            if (validated) {
              return@apply
            }
     ...
     */
    internal fun validity(): Int = (if (baseAmount.asKnown() == null) 0 else 1) + (baseCurrency.asKnown()?.validity() ?: 0) + (if (exponent.asKnown() == null) 0 else 1) + (if (rateString.asKnown() == null) 0 else 1) + (if (targetAmount.asKnown() == null) 0 else 1) + (targetCurrency.asKnown()?.validity() ?: 0) + (if (value.asKnown() == null) 0 else 1)
    ...
    override fun equals(other: Any?): Boolean {
      ...
      return other is ForeignExchangeRate && baseAmount == other.baseAmount && baseCurrency == other.baseCurrency && exponent == other.exponent && rateString == other.rateString && targetAmount == other.targetAmount && targetCurrency == other.targetCurrency && value == other.value && additionalProperties == other.additionalProperties
    }
    ...
    override fun toString() = "ForeignExchangeRate{baseAmount=$baseAmount, baseCurrency=$baseCurrency, exponent=$exponent, rateString=$rateString, targetAmount=$targetAmount, targetCurrency=$targetCurrency, value=$value, additionalProperties=$additionalProperties}"
}
