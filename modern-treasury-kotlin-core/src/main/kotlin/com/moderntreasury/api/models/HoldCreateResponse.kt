...
class HoldCreateResponse @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
    ...
    class Builder internal constructor() {
        ...
        internal fun from(holdCreateResponse: HoldCreateResponse) =
            apply {
     ...
     *   expected type.
     */
    fun validate(): HoldCreateResponse =
        apply {
            if (validated) {
              return@apply
            }
    ...
    override fun toString() = "HoldCreateResponse{id=$id, createdAt=$createdAt, object_=$object_, status=$status, targetId=$targetId, targetType=$targetType, updatedAt=$updatedAt, liveMode=$liveMode, metadata=$metadata, reason=$reason, resolution=$resolution, resolvedAt=$resolvedAt, additionalProperties=$additionalProperties}"
}
