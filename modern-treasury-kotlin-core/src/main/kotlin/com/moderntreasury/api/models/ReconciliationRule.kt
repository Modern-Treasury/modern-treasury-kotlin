...
class ReconciliationRule @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
     ...
     * @throws ModernTreasuryInvalidDataException if the JSON field has an unexpected type (e.g. if the server responded with an unexpected value).
     */
    fun customIdentifiers(): CustomIdentifiers? = customIdentifiers.getNullable("custom_identifiers")
    ...
    class Builder internal constructor() {
        ...
        internal fun from(reconciliationRule: ReconciliationRule) =
            apply {
        ...
        /** The lowest amount this expected payment may be equal to. Value in specified currency's smallest unit. e.g. $10 would be represented as 1000. */
        fun amountLowerBound(amountLowerBound: Long) = amountLowerBound(JsonField.of(amountLowerBound))
        ...
        /** The highest amount this expected payment may be equal to. Value in specified currency's smallest unit. e.g. $10 would be represented as 1000. */
        fun amountUpperBound(amountUpperBound: Long) = amountUpperBound(JsonField.of(amountUpperBound))
        ...
        /** A hash of custom identifiers for this payment */
        fun customIdentifiers(customIdentifiers: CustomIdentifiers?) = customIdentifiers(JsonField.ofNullable(customIdentifiers))
     ...
     *   expected type.
     */
    fun validate(): ReconciliationRule =
        apply {
            if (validated) {
              return@apply
            }
     ...
     */
    internal fun validity(): Int = (if (amountLowerBound.asKnown() == null) 0 else 1) + (if (amountUpperBound.asKnown() == null) 0 else 1) + (direction.asKnown()?.validity() ?: 0) + (if (internalAccountId.asKnown() == null) 0 else 1) + (if (counterpartyId.asKnown() == null) 0 else 1) + (currency.asKnown()?.validity() ?: 0) + (customIdentifiers.asKnown()?.validity() ?: 0) + (if (dateLowerBound.asKnown() == null) 0 else 1) + (if (dateUpperBound.asKnown() == null) 0 else 1) + (type.asKnown()?.validity() ?: 0)
