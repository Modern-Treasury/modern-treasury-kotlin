...
class Reversal @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
    ...
    class Builder internal constructor() {
        ...
        internal fun from(reversal: Reversal) =
            apply {
                id = reversal.id
                ...
                reconciliationStatus = reversal.reconciliationStatus
         ...
         *
         * You should usually call [Builder.reason] with a well-typed [Reason] value instead. This method is primarily for setting the field to an undocumented or not yet
         * supported value.
         */
     ...
     */
    internal fun validity(): Int = (if (id.asKnown() == null) 0 else 1) + (if (createdAt.asKnown() == null) 0 else 1) + (if (internalAccountId.asKnown() == null) 0 else 1) + (if (ledgerTransactionId.asKnown() == null) 0 else 1) + (if (liveMode.asKnown() == null) 0 else 1) + (metadata.asKnown()?.validity() ?: 0) + (if (object_.asKnown() == null) 0 else 1) + (if (paymentOrderId.asKnown() == null) 0 else 1) + (reason.asKnown()?.validity() ?: 0) + (reconciliationStatus.asKnown()?.validity() ?: 0) + (status.asKnown()?.validity() ?: 0) + (transactionIds.asKnown()?.sumOf { (if (it == null) 0 else 1).toInt() } ?: 0) + (if (updatedAt.asKnown() == null) 0 else 1)
    ...
    override fun equals(other: Any?): Boolean {
      ...
      return other is Reversal && id == other.id && createdAt == other.createdAt && internalAccountId == other.internalAccountId && ledgerTransactionId == other.ledgerTransactionId && liveMode == other.liveMode && metadata == other.metadata && object_ == other.object_ && paymentOrderId == other.paymentOrderId && reason == other.reason && reconciliationStatus == other.reconciliationStatus && status == other.status && transactionIds == other.transactionIds && updatedAt == other.updatedAt && additionalProperties == other.additionalProperties
    }

    private val hashCode: Int by lazy { Objects.hash(id, createdAt, internalAccountId, ledgerTransactionId, liveMode, metadata, object_, paymentOrderId, reason, reconciliationStatus, status, transactionIds, updatedAt, additionalProperties) }

    override fun hashCode(): Int = hashCode

    override fun toString() = "Reversal{id=$id, createdAt=$createdAt, internalAccountId=$internalAccountId, ledgerTransactionId=$ledgerTransactionId, liveMode=$liveMode, metadata=$metadata, object_=$object_, paymentOrderId=$paymentOrderId, reason=$reason, reconciliationStatus=$reconciliationStatus, status=$status, transactionIds=$transactionIds, updatedAt=$updatedAt, additionalProperties=$additionalProperties}"
}
