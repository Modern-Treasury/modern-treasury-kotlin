...
class RoutingDetail @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
    ...
    private constructor(
        ...
        @JsonProperty("bank_address") @ExcludeMissing bankAddress: JsonField<Address> = JsonMissing.of(),
    ...
    class Builder internal constructor() {
         ...
         *
         * You should usually call [Builder.bankAddress] with a well-typed [Address] value instead. This method is primarily for setting the field to an undocumented or not yet
         * supported value.
         */
        fun bankAddress(bankAddress: JsonField<Address>) =
            apply {
     ...
     */
    internal fun validity(): Int = (if (id.asKnown() == null) 0 else 1) + (bankAddress.asKnown()?.validity() ?: 0) + (if (bankName.asKnown() == null) 0 else 1) + (if (createdAt.asKnown() == null) 0 else 1) + (if (discardedAt.asKnown() == null) 0 else 1) + (if (liveMode.asKnown() == null) 0 else 1) + (if (object_.asKnown() == null) 0 else 1) + (paymentType.asKnown()?.validity() ?: 0) + (if (routingNumber.asKnown() == null) 0 else 1) + (routingNumberType.asKnown()?.validity() ?: 0) + (if (updatedAt.asKnown() == null) 0 else 1)
    ...
    override fun equals(other: Any?): Boolean {
      ...
      return other is RoutingDetail && id == other.id && bankAddress == other.bankAddress && bankName == other.bankName && createdAt == other.createdAt && discardedAt == other.discardedAt && liveMode == other.liveMode && object_ == other.object_ && paymentType == other.paymentType && routingNumber == other.routingNumber && routingNumberType == other.routingNumberType && updatedAt == other.updatedAt && additionalProperties == other.additionalProperties
    }
    ...
    override fun toString() = "RoutingDetail{id=$id, bankAddress=$bankAddress, bankName=$bankName, createdAt=$createdAt, discardedAt=$discardedAt, liveMode=$liveMode, object_=$object_, paymentType=$paymentType, routingNumber=$routingNumber, routingNumberType=$routingNumberType, updatedAt=$updatedAt, additionalProperties=$additionalProperties}"
}
