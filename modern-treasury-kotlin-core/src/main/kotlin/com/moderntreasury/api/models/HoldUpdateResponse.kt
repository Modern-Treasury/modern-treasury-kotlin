...
class HoldUpdateResponse @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
    ...
    class Builder internal constructor() {
        ...
        internal fun from(holdUpdateResponse: HoldUpdateResponse) =
            apply {
     ...
     *   expected type.
     */
    fun validate(): HoldUpdateResponse =
        apply {
            if (validated) {
              return@apply
            }
    ...
    override fun toString() = "HoldUpdateResponse{id=$id, createdAt=$createdAt, object_=$object_, status=$status, targetId=$targetId, targetType=$targetType, updatedAt=$updatedAt, liveMode=$liveMode, metadata=$metadata, reason=$reason, resolution=$resolution, resolvedAt=$resolvedAt, additionalProperties=$additionalProperties}"
}
