...
class RoutingNumberLookupRequest @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
     ...
     * @throws ModernTreasuryInvalidDataException if the JSON field has an unexpected type (e.g. if the server responded with an unexpected value).
     */
    fun routingNumberType(): RoutingNumberType? = routingNumberType.getNullable("routing_number_type")
     ...
     * @throws ModernTreasuryInvalidDataException if the JSON field has an unexpected type (e.g. if the server responded with an unexpected value).
     */
    fun supportedPaymentTypes(): List<SupportedPaymentType>? = supportedPaymentTypes.getNullable("supported_payment_types")
    ...
    fun validate(): RoutingNumberLookupRequest =
        apply {
            ...
            sanctions()?.validate()
            supportedPaymentTypes()?.forEach { it.validate() }
            validated = true
        }
     ...
     */
    internal fun validity(): Int = (bankAddress.asKnown()?.validity() ?: 0) + (if (bankName.asKnown() == null) 0 else 1) + (if (routingNumber.asKnown() == null) 0 else 1) + (routingNumberType.asKnown()?.validity() ?: 0) + (sanctions.asKnown()?.validity() ?: 0) + (supportedPaymentTypes.asKnown()?.sumOf { it.validity().toInt() } ?: 0)
