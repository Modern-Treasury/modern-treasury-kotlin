...
class ContactDetail @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
    ...
    /** @throws ModernTreasuryInvalidDataException if the JSON field has an unexpected type or is unexpectedly missing or null (e.g. if the server responded with an unexpected value). */
    fun contactIdentifierType(): ContactIdentifierType = contactIdentifierType.getRequired("contact_identifier_type")
    ...
    class Builder internal constructor() {
        ...
        internal fun from(contactDetail: ContactDetail) =
            apply {
                id = contactDetail.id
     ...
     */
    internal fun validity(): Int = (if (id.asKnown() == null) 0 else 1) + (if (contactIdentifier.asKnown() == null) 0 else 1) + (contactIdentifierType.asKnown()?.validity() ?: 0) + (if (createdAt.asKnown() == null) 0 else 1) + (if (discardedAt.asKnown() == null) 0 else 1) + (if (liveMode.asKnown() == null) 0 else 1) + (if (object_.asKnown() == null) 0 else 1) + (if (updatedAt.asKnown() == null) 0 else 1)
    ...
    override fun toString() = "ContactDetail{id=$id, contactIdentifier=$contactIdentifier, contactIdentifierType=$contactIdentifierType, createdAt=$createdAt, discardedAt=$discardedAt, liveMode=$liveMode, object_=$object_, updatedAt=$updatedAt, additionalProperties=$additionalProperties}"
}
